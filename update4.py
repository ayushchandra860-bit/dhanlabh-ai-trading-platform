...
# Replace the Signal Confirmation Gate block with the Smart Refresh Policy
refresh_pattern = r"(      // ── Signal Confirmation Gate ────────────────────────────────────────.*?)(?=      setAIState\('IDLE'\);)"
...
if refresh_match:
    ...
    new_body = """      // ── Smart Overlay Refresh Policy ────────────────────────────────────────
      ...
      // We will define a meaningful change based on the user's constraints
      ...
      if (shouldUpdate) {
        ...
        // Signal Persistence stub: we will save this to disk or DB if it's a BUY/SELL
        if (currentSignal === 'BUY' || currentSignal === 'SELL') {
           // We will implement signal history persistence here
           try {
             const fs = require('fs');
