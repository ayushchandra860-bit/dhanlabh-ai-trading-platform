...
app.whenReady().then(async () => {
  ...
  try {
    ...
    if (result) {
       ...
       report += `## 2. Chart Detection\n- **Expected:** Chart bounds identified dynamically\n- **Actual:** Detected boundaries via pngjs (Confidence: ${result.analysisData?.visionConfidence}%). Region: x=${result.chartBounds?.x}, y=${result.chartBounds?.y}, w=${result.chartBounds?.width}, h=${result.chartBounds?.height}\n- **Status:** ${result.chartBounds ? 'PASS' : 'FAIL'}\n\n`;
       ...
       report += `*Note: Validation executed using artifact media__1784385136412.png.*\n`;
